...
namespace dlovi {
    ...
    Matrix Matrix::conv2(const Matrix & rhs, const ConvolutionType type) const {
        ...
        if (type==CONV_FULL) {
                ...
                for (int j = 0; j < retVal.m_nCols; j++) {
                    ...
                    for (int p = 1; p <= rhs.m_nRows; p++) {
                        for (int q = 1; q <= rhs.m_nCols; q++) {
                            ...
                            if (nRowIndex >= 0 && nRowIndex < m_nRows && nColIndex >= 0 && nColIndex < m_nCols) {
                                // use zero to pad
            ...
            // TODO: invalid type.  should not happen.  throw exception.
        }
        return retVal;
    }
